...
test('Enrich - stops the whole run once every model is quota-exhausted, instead of retrying dead models per batch', async () => {
  ...
  // the time batch 1 exhausts all 6 models, batches 2 and 3 must be skipped entirely
  // -- not attempted at all -- rather than each burning through 6 more doomed calls.
  ...
  await enrichMissingArtistMetadata(artists, file, 'fake-key', generateFn);

  // Exactly one model-exhaustion pass (6 calls) for batch 1, then early exit --
  // not 3 batches x 6 models = 18 calls.
  assert.strictEqual(callCount, 6, `expected exactly 6 calls (one full model sweep before exhaustion), got ${callCount}`);
...
test('Enrich - a model that already failed with a quota error is not retried on the next batch', async () => {
  ...
  const generateFn: GenerateEnrichmentFn = async ({ modelName }) => {
    ...
    if (modelName === 'gemini-3.5-flash') throw quotaError();
    return JSON.stringify([]);
  };

  await enrichMissingArtistMetadata(artists, file, 'fake-key', generateFn);

  assert.strictEqual(callsByModel['gemini-3.5-flash'], 1, 'the quota-exhausted model should only be tried once across the whole run');
  assert.strictEqual(callsByModel['gemini-3.1-flash'], 2, 'the working model should be tried once per batch (2 batches)');
...
test('Enrich - saves progress after every batch, not only at the end', async () => {
  ...
  const generateFn: GenerateEnrichmentFn = async ({ modelName }) => {
    if (modelName !== 'gemini-3.5-flash') throw new Error('should not reach this model');
