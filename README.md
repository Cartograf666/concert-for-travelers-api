...
- **`artists/{artist-slug}.json`** — concerts for one artist (e.g. `artists/the-cure.json`), sorted by date. **Note**: Because this API is statically hosted on GitHub Pages, non-existent artist files will return an HTML 404 instead of JSON. Consumers should check the requested slug against the `artists` array in `index.json` before querying.
- **`cities/{city-slug}.json`** — concerts for one city (e.g. `cities/berlin.json`), sorted by date. Stale per-slug files for artists/cities no longer touring are pruned every run. **Note**: Same as above, consumers should check the requested slug against the `cities` array in `index.json` before querying.
