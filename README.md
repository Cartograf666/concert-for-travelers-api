...
- **`artists/{artist-slug}.json`** — concerts for one artist (e.g. `artists/the-cure.json`), sorted by date. **Note**: Because this API is statically hosted on GitHub Pages, non-existent artist files will return an HTML 404 instead of JSON. `index.json`'s `artists` array holds raw display names, not slugs — `slugify()` each name first, then check the requested slug against that derived set before querying (comparing the raw slug directly against the `artists` array won't match).
- **`cities/{city-slug}.json`** — concerts for one city (e.g. `cities/berlin.json`), sorted by date. Stale per-slug files for artists/cities no longer touring are pruned every run. **Note**: Same as above — `slugify()` each name in `index.json`'s `cities` array before comparing.
- **`status.json`** — machine-readable health surface for the dashboard and freshness watchdog. Version 3 retains the existing counters and adds `sourceHealth` (venue, artist tour pages, Bandsintown, Eventbrite, Ticketmaster) and `processing` (accepted, duplicate and rejected counts by reason/source). Source verification uses explicit successful checks, including HTTP 304; retained legacy cache without `verifiedAt` is **unknown**, not freshly verified. API snapshot time is separate from source age. Configured revisit intervals are request constraints, not a freshness promise for every target. Processing counts describe that pass; `publication.state` records gate eligibility, not a confirmed Pages deployment.
...
The dashboard shows source failures, cache fallback, partial coverage and the next
diagnostic action. Missing/old reports remain explicit. Detailed rejection samples
are bounded and stored only in `reports/processing-diagnostics.json` in workflow
artifacts; the public status contains aggregate counts without those samples.
Collection schedules, provider request limits and event normalization are unchanged.
